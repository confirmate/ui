...
export default function EnableCatalogButton({
  ...
  return (
    <li className="flex flex-col space-y-4 text-sm">
      <div>
        <div className="font-medium text-gray-900">
          Configure new audit scope
        </div>

        <div className="text-gray-500">
          Please select a catalog to create a new audit scope.
        </div>
      </div>
      <select
        className="
    ...
    focus:ring-confirmate sm:text-sm sm:leading-6"
        ...
        onChange={(e) => {
          const catalog = leftOverCatalogs.find(
            (catalog) => catalog.id === e.target.value,
          );
          if (catalog) {
            setCatalog(catalog);
          }
