...
export default async function TargetOfEvaluationCard({
  ...
  function buildData(
    compliance: Map<string, Map<string, ComplianceStatus>>,
    ...
    return {
      ...
      datasets: [
        {
          label: "Compliant",
          data: Array.from(compliance.values()).map((entry) =>
            percentage(entry, [
        ...
        },
        {
          label: "Pending",
          data: Array.from(compliance.values()).map((entry) =>
            percentage(entry, ["EVALUATION_STATUS_PENDING"]),
          ),
  ...
  return (
    <li
      key={target.id}
      className="overflow-hidden shadow-sm rounded-xl border border-gray-200"
    >
        ...
        <div className="flex justify-between gap-x-4 py-3">
          ...
          <dd className="flex items-start gap-x-2">
            <div className="font-medium text-gray-900">
              {statistics?.numberOfAssessmentResults}
            </div>
          </dd>
        </div>
        <div className="flex justify-between gap-x-4 py-3">
          ...
          <dd className="flex items-start gap-x-2">
            <div className="font-medium text-gray-900">
              {statistics?.numberOfSelectedCatalogs}
            </div>
          </dd>
        </div>
...
function percentage(
  entry: Map<string, ComplianceStatus>,
  statuses: string[],
): number {
  return (
    Array.from(entry.values()).filter((value) => statuses.includes(value))
      .length / entry.size
  );
}
